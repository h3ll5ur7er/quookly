...
class Entry(NamedTuple):
    ...
    reference: str
    #: Where the published table says this food sits, as the slug of a leaf in the tree
    #: below. Absent where the row names no category, which a handful do not.
    category: str | None


class CategoryEntry(NamedTuple):
    """One node of the tree, named in every edition that publishes it."""
...
#: A category path is at most two deep, and the separator is not a reliable one below the
#: first level: *Savoury snacks/Chips/Crisps* and *Sweets/Cookies/Biscuits* are British and
#: American names for one group, not a third level. So the first slash separates the
#: section from the group, and everything after it is the group's name verbatim.
def _split_category(path: str) -> tuple[str, str | None]:
    section, _, group = path.partition("/")
    return section.strip(), group.strip() or None


def _category_slug(section: str, group: str | None) -> str:
    """A stable key for a node, derived from the English path.

    English rather than the reader's language, for the same reason a slug is: it is an
    identity, and an identity that changes with the language it is read in is not one.
    """
    return _slug(section) if group is None else f"{_slug(section)}-{_slug(group)}"


def _categories(tables: dict[str, dict[str, tuple[Any, ...]]]) -> list[CategoryEntry]:
    """The tree the three editions publish, in the order they publish it.

    Free, and the reason this is worth doing at all: the row ids are identical across the
    English, German and French workbooks, so every category comes out named in all three
    without anybody translating anything (FR-10).

    Only the *first* category of a row is taken. Forty-seven rows name two — apple juice is
    a fruit juice and a non-alcoholic beverage — and a shopping list that puts one thing in
    two aisles is a shopping list with a thing in it twice.
    """
    found: dict[str, CategoryEntry] = {}
    for row_id, row in tables["en-GB"].items():
        published = str(row[CATEGORY] or "").split(";")[0].strip()
        if not published or published == "Category":
            continue
        section, group = _split_category(published)

        names: dict[str, dict[str, str]] = {"section": {}, "group": {}}
        for locale, table in tables.items():
            localised = str(table[row_id][CATEGORY] or "").split(";")[0].strip()
            if not localised:
                continue
            their_section, their_group = _split_category(localised)
            names["section"][locale] = their_section
            if their_group is not None:
                names["group"][locale] = their_group

        section_slug = _category_slug(section, None)
        if section_slug not in found:
            found[section_slug] = CategoryEntry(section_slug, None, names["section"])
        if group is not None:
            group_slug = _category_slug(section, group)
            if group_slug not in found:
                found[group_slug] = CategoryEntry(group_slug, section_slug, names["group"])
    # Sections before the groups that hang off them: a parent has to exist to be pointed at.
    return sorted(found.values(), key=lambda one: (one.parent is not None, one.slug))
...
def main() -> None:
    ...
    for row_id, row in ordered:
        ...
        section, group = _split_category(first) if first else ("", None)
        entries.append(
            Entry(
                row=int(row_id),
                slug=slug,
                ...
                category=_category_slug(section, group) if section else None,
            )
        )
        ...
        json.dumps(
            {
                ...
                "categories": [one._asdict() for one in _categories(tables)],
    ...
    placed = sum(1 for e in entries if e.category)
    print(f"  {placed} sit in one of {len(_categories(tables))} categories")
