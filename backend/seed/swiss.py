...
PLACEMENTS = Path(__file__).parent / "placements.json"

#: The category column, by position. The same one `generic.py` reads, and identical across
#: the three published editions.
CATEGORY = 5
...
def _category_slug(path: str) -> str:
    """The slug `generic.py` gives this category path, so both builders agree.

    Deliberately the same two rules: the first slash separates the section from the group,
    and everything after it is the group's name verbatim — *Sweets/Cookies/Biscuits* is one
    group with two names, not two levels.
    """
    section, _, group = path.partition("/")
    head = _slugify(section.strip())
    tail = group.strip()
    return head if not tail else f"{head}-{_slugify(tail)}"


def _slugify(text: str) -> str:
    folded = unicodedata.normalize("NFKD", text.lower())
    stripped = "".join(one for one in folded if not unicodedata.combining(one))
    return re.sub(r"-+", "-", re.sub(r"[^a-z0-9]+", "-", stripped)).strip("-")
...
def main() -> None:
    ...
    for slug, entry in sorted(MAPPED.items()):
        ...
        # Where the table puts it. The hand-written mapping above already says which
        # published row answers for this slug, so the category comes free and correct —
        # and without it the entries a *recipe* actually names, which are these, would be
        # the only ones the shopping list could not put in an aisle (ADR-067).
        published_category = str(row[CATEGORY] or "").split(";")[0].strip()
        if published_category:
            placements.append({"slug": slug, "category": _category_slug(published_category)})
        amounts = {
            nutrient: value
        ...
        json.dumps(
            {
                ...
                "retrieved": datetime.now(UTC).date().isoformat(),
                "note": (
                    "Where the Swiss table puts the ingredients this application maps by hand. "
                    "Generated by seed/swiss.py: do not edit."
                ),
    ...
    print(f"{len(placements)} placements -> {PLACEMENTS}")
