...
| `GenerationEngine` | V1, V3 | Composes prompts for synthesis; parses and constrains model output. Knows *what to ask*, never *whom to ask* — and never what an answer *means*: the shape that comes back is the shape a page's recipe comes back in, read by `InterpretationEngine`. **Built** for UC-1.4 and UC-1.5 ([ADR-047](07-decisions.md#adr-047-a-generated-recipe-is-refused-not-warned-about)). |
